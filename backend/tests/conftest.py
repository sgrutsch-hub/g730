...
async def app(db: AsyncSession):
    """Create a test app with overridden database dependency."""
...
async def test_user(db: AsyncSession) -> User:
    """Create a test user with a default profile."""
    user = User(
...
async def test_profile(db: AsyncSession, test_user: User) -> Profile:
    """Get the test user's default profile."""
...
async def test_session_with_shots(
    ...
    """Create a session with realistic 7 Iron shots."""
    session = Session(
